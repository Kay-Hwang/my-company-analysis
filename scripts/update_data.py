...
def get_indicators():
    indicator_data = {"latest": {"indices": {}, "inflation": {}, "labor": {}, "interest_rates": {}}, "history": {}}
    if not FRED_API_KEY:
    ...
    for key, series_info in all_series.items():
        ...
        try:
            ...
            # 1. 과거 데이터 저장
            indicator_data['history'][series_id] = [{"date": obs['date'], "value": obs['value']} for obs in observations]
            ...
            if observations:
                ...
                for group, items in FRED_SERIES.items():
                    if key in items:
                        ...
                        break
                if found_group:
                    indicator_data['latest'][found_group][key] = {
                 ...
                 print(f"No valid observations for {key}")
